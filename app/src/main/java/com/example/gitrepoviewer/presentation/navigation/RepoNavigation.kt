...
fun RepoNavigation(){
    ...
    NavHost(navController = navController, startDestination = RepoScreens.ReposScreen.name){
        ...
        composable(RepoScreens.RepoIssuesScreen.name + "/{ownerName}" + "/{repoName}" + "/{repoId}", arguments = listOf(navArgument(name = "ownerName") {type = NavType.StringType}, navArgument(name = "repoName") {type = NavType.StringType}, navArgument(name = "repoId") {type = NavType.LongType})){ backStackEntry ->
            RepoIssuesScreen(navController = navController)
        }
    }
}